...
const RoomCard = ({ room }) => {
  return (
    <div className="group bg-[#F5E6BE] rounded-[2.5rem] overflow-hidden shadow-premium hover:shadow-[0_50px_100px_-20px_rgba(58,41,23,0.25)] transition-all duration-700 border border-primary-brown/10 cursor-pointer relative">
      <Link to={`/rooms/${room.id}`} className="relative h-80 overflow-hidden block">
        <img
          src={room.image}
          alt={room.name}
        ...
        <div className="absolute top-6 right-6 glass px-6 py-2 rounded-full text-primary-brown font-black text-sm shadow-xl border-white/40">
          <span className="font-accent italic text-lg opacity-80">${room.price}</span> 
      ...
      <div className="p-10">
        ...
        <div className="flex items-center space-x-6 mb-10 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">
          <div className="flex items-center">
            <Users className="w-3 h-3 mr-2 text-primary-olive" />
            {room.capacity} Huéspedes
          </div>
          <div className="flex items-center border-l border-primary-brown/10 pl-6">
            <BedDouble className="w-3 h-3 mr-2 text-primary-olive" />
            {room.bedType}
          </div>
        </div>
        ...
        <Link
          to={`/rooms/${room.id}`}
          className="relative block w-full text-center bg-primary-brown text-white py-5 rounded-2xl font-black uppercase text-[10px] tracking-[0.3em] hover:bg-primary-olive transform active:scale-95 transition-all duration-500 shadow-xl overflow-hidden group/btn"
        >
